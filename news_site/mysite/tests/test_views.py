...
class CommentViewTest(TestCase):
    ...
    def test_comment_update_view(self):
        test_response = self.client.get('/comment/3/update')
        self.assertEqual(test_response.status_code, 200)
        self.assertTrue('comment' in test_response.context)
        self.assertTemplateUsed(test_response, 'mysite/actions/comments/update.html')
        self.assertEqual(test_response.context['comment'].id, 3)
        self.assertEqual(test_response.context['comment'].text, 'Comment_Text1')
        null_response = self.client.get('/comment/9/update')
        self.assertEqual(null_response.status_code, 404)

    def test_comment_delete_view(self):
        test_response = self.client.get('/comment/2/delete')
        self.assertEqual(test_response.status_code, 200)
        self.assertTrue('comment' in test_response.context)
        self.assertTemplateUsed(test_response, 'mysite/actions/comments/delete.html')
        self.assertEqual(test_response.context['comment'].id, 2)
        self.assertEqual(test_response.context['comment'].text, 'Comment_Text2')
        null_response = self.client.get('/comment/9/delete')
        self.assertEqual(null_response.status_code, 404)
